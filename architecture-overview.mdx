...
description: "Overview of the CLI / plugin architecture in alpha 0.1.1"
---
...
**## System Map (How CLI commands are ran)**
...
2. **The registry resolves the command** using explicit namespaces `wormhole:quote`), the active protocol `use 1inch`), or fuzzy matching (fuzzy matching is not enabled currently).
...
(for the maths, check out Algebraic Core)
